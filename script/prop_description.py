...
def get_property_attr_query(property):
    property_attr_query = \
        'SELECT DISTINCT ?desc ?prop_label WHERE \
        {\
          wd:' + property + ' schema:description ?desc.\
          filter (lang(?desc)="en")\
          wd:P1546 rdfs:label ?prop_label.\
...
def fill_description(desc_template, data):
    ...
    desc_template['description'] = data['']
...
def process_property_attr_query(desc_template, data):
    ...
    for item in data:
        ...
        variable.setdefault('semantic_type', [])
                .append(item['type']['value'])
...
if __name__ == '__main__':
    ...
    property_attr_query_encoded = encoded_url(get_property_attr_query)
    identifier_attr_query_encoded = encoded_url(get_identifier_attr_query)
    ...
    desc_template = process_property_attr_query(desc_template, get_query_result(property_attr_query))

    desc_template = property_attr_query_encoded(desc_template, get_query_result(identifier_attr_query))
